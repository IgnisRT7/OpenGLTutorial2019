...
*	画面の情報
*
*	x: 1.0 / ウインドウの幅(ピクセル単位)
*	y: 1.0 / ウインドウの高さ(ピクセル単位)
*	x: near(m単位)
*	y: far(m単位)
*/
...
vec2 CalcCoC(float objectDistance){
	...
	return abs(aperture * (focalLength * (focalPlane - objectDistance)) / (objectDistance * (focalPlane - focallength))) * mmToPixel * viewInfo.xy;
}
...
void main(){
	...
	float centerZ = ToRealZ(texColorArray[0], inTexCoord).r);
