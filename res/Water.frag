...
void main(){
	...
	vec3 normalL = matTBN * (texture(texNormalArray[0],uv.xy + scroll.zw).rgb * 2.0 -1.0);
  ...
  vec3 enviromentColor=texture(texCubeMap,reflectionVector).rgb;
  fragColor.rgb += enviromentColor;
