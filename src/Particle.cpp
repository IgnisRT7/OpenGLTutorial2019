...
ParticleEmitter::ParticleEmitter(const ParticleEmitterParameter& ep, const ParticleParameter& pp) :
	ep(ep),
	pp(pp),
	interval(1.0f / ep.emissionPerSecond) {
...
void ParticleEmitter::Draw() {
	...
	if (count) {
		...
		glDrawElementsBaseVertex(GL_TRIANGLES, count, GL_UNSIGNED_SHORT, reinterpret_cast<const GLvoid*>(0), baseVertex);
	}
}
...
bool ParticleSystem::Init(size_t maxParticleCount) {
	...
	if (!vbo.Create(GL_ARRAY_BUFFER, sizeof(Vertex) * maxParticleCount * 4, nullptr, GL_STREAM_DRAW)) {
		std::cerr << "[エラー]: パーティクル・システムの初期化に失敗しました\n";
		return false;
	}
	...
	if (!ibo.Create(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort) * indices.size(), indices.data(), GL_STATIC_DRAW)) {
...
*	@param id 検索するID
*
*	@retval !nullptr	引数IDと一致するIDを持つエミッター
...
void ParticleSystem::Update(float deltaTime) {
	...
	for (auto& e : emitters) {
		...
		for (auto& particle : e->particles) {
			...
			const Rect& rect = particle.rect;	//読みやすくする短目の参照を定義
...
void ParticleSystem::Draw(const glm::mat4& matProj, const glm::mat4& matView) {
	...
	glDepthMask(false);	//深度バッファへの書き込みをやめる
	...
	//シェーダにパラメータを設定
	program->Use();
	program->SetInverseViewRotationMatrix(matProj * matView);
